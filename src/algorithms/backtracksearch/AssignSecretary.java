...
public class AssignSecretary {
	...
	private void updateBest(){
		...
		if(Utility.compare(bestEval,e) == 1){
			System.out.println("AssignSecretary::updateBest, bestEval = " + bestEval[0] + "," + bestEval[1] + ", newEval = " + e[0] + "," + e[1]);
	...
	private void TRY(int k){
		...
		for(int i = 0; i < domain[k].size(); i++){
			...
			//System.out.println("TRY(" + k + "), v = " + v + ", rid =  " + rid + ", hustOfRoom.get(rid).size() = " + 
			//hustOfRoom.get(rid).size() + ", ok = " + ok);
	...
	public void search(Vector<JuryInfo> jury, Vector<Room> rooms, HashMap[] matchScore, Vector<Teacher> hustTeachers, int timeLimit){
		...
		for(int i = 0; i < n; i++){
			...
			if(J.getSecretaryId() > 0){
				...
				for(int k = 0; k < sz; k++)
					domain[i].add(a[k]);
				System.out.println("AssignSecretary: domain[" + i + "] = " + domain[i].size());
			}
		}
		...
		System.out.println("AssignSecretary::search Start TRY.....");
		TRY(0);
		System.out.println("AssignSecretary::search TRY finished.....");
		
		for(int i = 0; i < n; i++){
			JuryInfo J = jury.get(i);
			J.setSecretaryId(opt_x[i]);
		}
