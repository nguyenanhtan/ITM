...
public class AssignExaminer2 {
	...
	private boolean balance(Vector<JuryInfo> jury, HashMap<Integer, Integer> mOccE2, HashMap<Integer, Integer> mOccA){
		for(int i = 0; i < jury.size(); i++){
			JuryInfo J = jury.get(i);
			int e2 = J.getExaminerId2();
			int s = J.getSecretaryId();
			//if(Math.abs(mOccE2.get(e2)-mOccE2.get(s)) > 1){
			if(mOccE2.get(e2)-mOccE2.get(s) > 1){
				J.setExaminerId2(s);
				J.setSecretaryId(e2);
				mOccE2.put(e2, mOccE2.get(e2)-1);
				mOccE2.put(s, mOccE2.get(s)+1);
				System.out.println("AssignExaminer2::balance --> swap " + e2 + " and " + s + " in jury " + i);
				//return true;
			}
		}
		return false;
	}
	public void balances(Vector<JuryInfo> jury, Vector<Teacher> hustTeachers){
		this.jury = jury;
		//this.nonHust = nonHustTeachers;
		HashMap<Integer, Integer> mOccE2 = new HashMap<Integer, Integer>();
		HashMap<Integer, Integer> mOccS = new HashMap<Integer, Integer>();
		for(int i = 0; i < jury.size(); i++){
			JuryInfo J = jury.get(i);
			int e2 = J.getExaminerId2();
			int s = J.getSecretaryId();
			if(mOccE2.get(e2) == null) mOccE2.put(e2, 1);
			else mOccE2.put(e2, mOccE2.get(e2)+1);
			if(mOccS.get(e2) == null) mOccS.put(e2, 0);
			
			if(mOccS.get(s) == null) mOccS.put(s, 1);
			else mOccS.put(s, mOccS.get(s)+1);
			if(mOccE2.get(s) == null) mOccE2.put(s, 0);
		}
		balance(jury,mOccE2,mOccS);
		/*
		int count = 0;
		while(true && count <= 30){
	...
	public void search(Vector<JuryInfo> jury, Vector<Room> rooms, HashMap[] matchScore, Vector<Teacher> hustTeachers, int timeLimit){
		...
		for(int i = 0; i < n; i++){
			...
			if(J.getExaminerId2() > 0){
				...
				for(int j = 0; j < hust.size(); j++){
					...
					if(J.getPresidentId() > 0 && J.getPresidentId() == hust.get(j).getID()) ok = false;
		...
		System.out.println("AssignExaminer2::search Start TRY.....");
		TRY(0);
		System.out.println("AssignExaminer2::search TRY finished.....");
		
		for(int i = 0; i < n; i++){
			JuryInfo J = jury.get(i);
			J.setExaminerId2(opt_x[i]);
		}
